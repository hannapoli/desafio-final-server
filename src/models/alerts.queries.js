...
const alertsQueries = {
  ...
  getAllAlertsByUser: `SELECT DISTINCT ON (alertas.uid_parcel) *
    ...
    `,
  getAllInfoMeteoByUser: `SELECT DISTINCT ON (mf.uid_parcel) *
  ...
  getParcelVegetationByParcelID: `SELECT *
    FROM parcel_vegetation_indices WHERE uid_parcel = $1`,
  getParcelCropsByParcelID: `SELECT c. *
    FROM parcels p
    INNER JOIN cultivos c
    ON p.id_cultivo = c.id_cultivo
    WHERE p.uid_parcel = $1`,
}
